...
const CardJournals = ({
  ...
  return (
    <div className={'p-4'}>
      ...
      >
        {!loading && journals.map((item, index) => (
          <li
            key={item.id}
          ...
          >
              <Link href={`/journals/journals-view/?id=${item.id}`} className='text-lg font-bold leading-6 line-clamp-1'>
                  {item.title}
              </Link>
              ...
              <div className='ml-auto'>
                <ListActionsPopover
                  onDelete={onDelete}
                  itemId={item.id}
                  pathEdit={`/journals/journals-edit/?id=${item.id}`}
                  pathView={`/journals/journals-view/?id=${item.id}`}
